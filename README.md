...
## Timestamps
Timestamps are issued in recorder - it represents recieved time since the recording started. If you need to measure the sender's time, I suggest you add extra parameter to your OSC message that will represent time send in millis. OSC does not natively supports synchronized clock between server and client and bundles' timatag is only releative to reciever absolute time.  See the [specification](https://opensoundcontrol.stanford.edu/spec-1_0.html) for more. 
